...
export const ozonCrawlerTask = cron.schedule(OZON_CRAWLER_CRON, async () => {
  const crawler = new OZONCrawler().createCrawler(OZON_MAX_REQUESTS, OZON_MAX_CONCURRENCY, OZON_SCROLL_TIMES, OZON_TIME_BETWEEN_SCROLLS);
  const ozonFiles = new OZONFiles();

  await crawler.run([ OZON_SELLER_URL ]);
